...
from .tagging_heads.bert import BertConfig, BertModel
from .tagging_heads.asymmetric_loss import AsymmetricLoss

from .CAR_heads.TQN_model import TQN_Model
...
class CLIP(nn.Module):
    ...
    def __init__(
            ...
            prompt_template_setting: Optional[str] = None,
        ...
        self.prompt_template_setting = prompt_template_setting
        ...
        #region tagging head
        tag_bert_config = BertConfig.from_json_file("tagging_heads/tag_bert_config.json")
        ...
        for layer in self.tag_head.encoder.layer:
            del layer.attention
        tag_list = "tagging/scar_tag_list.txt"
    ...
    def lock_text_tower(self, unlocked_groups: int = 0, freeze_layer_norm: bool = True):
        self.text.lock(unlocked_groups, freeze_layer_norm)
    ...
    def encode_image(self, image, normalize: bool = False):
        features, token = self.visual(image)
        return F.normalize(features, dim=-1) if normalize else features, token
    ...
    def encode_text(self, text, normalize: bool = False): 
        ...
        projected_feature = text_global_pool(x, text, self.text_pool_type)
                
        if self.text_projection is not None:
            if isinstance(self.text_projection, nn.Linear):
                projected_feature = self.text_projection(x)
            else:
                projected_feature = projected_feature @ self.text_projection
    ...
    def tag_forward(self, tag_embeds):
        bs = len(tag_embeds)
    ...
    def prepare_control_words(self, tag_logits, samples=[]):
        control_words = []
        full_drop_ratio = 0.5
        drop_ratio =  0.5
        top_k = 1
        categroy_size = [3,4,3,4,4,4]
        tags_num = 22

        tag_scores = tag_logits.sigmoid()
        current_pos = 0
        for i, size in enumerate(categroy_size):
            category_idxs = torch.argsort(tag_scores[:,current_pos:current_pos+size] + tag_scores[:,tags_num+current_pos:tags_num+current_pos+size], dim=-1, descending=True)[:, :top_k]
            if i == 0:
                tag_idxs = category_idxs
            else:
                tag_idxs = torch.cat([tag_idxs, category_idxs+current_pos], dim=-1)
            current_pos += size
        

        tags = [[self.tag_list[tag_idx] for tag_idx in tag_idxs[bz_idx]]
    ...
    def forward(
            ...
            additional_label: torch.Tensor,
            tokenizer: torch.nn.Module,
            class_words: list,
    ):
        image_features, out_token = self.encode_image(image, normalize=True)

        tag_logits = self.tag_forward(out_token)
        
        tagging_words = self.prepare_control_words(tag_logits)
        batch_psudo_prompts = []
        
        if self.use_tagging:
            templates = ["sentence_1", "sentence_2", "sentence_3", "sentence_4","itemization"]
            if self.prompt_template_setting == "sentence_1":
                selected_template = "sentence_1"
            elif self.prompt_template_setting == "sentence_2":
                selected_template = "sentence_2"
            elif self.prompt_template_setting == "sentence_3":
                selected_template = "sentence_3"
            elif self.prompt_template_setting == "sentence_4":
                selected_template = "sentence_4"
            elif self.prompt_template_setting == "itemization":
                selected_template = "itemization"
            elif self.prompt_template_setting == "total":
                selected_template = random.choice(templates)
            else:
                raise ValueError("Invalid prompt template setting. Choose from 'sentence_1', 'sentence_2', 'sentence_3', 'itemization', or 'total'.")
                
            for batch_idx, (class_word, batch_tags_str) in enumerate(zip(class_words,tagging_words)) if class_words is not None else enumerate(tagging_words):
                tags = batch_tags_str.split(",")
                if selected_template == "sentence_1":
                    psudo_prompt = f"A {class_word} with a {tags[0]}, exhibiting a {tags[1]} and {tags[2]}. It has a {tags[3]}, with {tags[4]} and {tags[5]}."
                elif selected_template == "sentence_2":
                    psudo_prompt = f"This is an image of {class_word} with a {tags[0]}, exhibiting a {tags[1]} and {tags[2]}. It has a {tags[3]}, with {tags[4]} and {tags[5]}."
                elif selected_template == "sentence_3":
                    psudo_prompt = f"{class_word} with a {tags[0]}, exhibiting a {tags[1]} and {tags[2]}. It has a {tags[3]}, with {tags[4]} and {tags[5]} presented in image"
                elif selected_template == "sentence_4":
                    psudo_prompt = f"a photo of {class_word} with a {tags[0]}, exhibiting a {tags[1]} and {tags[2]}. It has a {tags[3]}, with {tags[4]} and {tags[5]}."
                elif selected_template == "itemization":
                    psudo_prompt = f"A {class_word}, Width: {tags[0]}, Color: {tags[1]}, Pigmentation: {tags[2]}, Surface: {tags[3]}, Irregular Color: {tags[4]}, Irregular Height: {tags[5]}."                    
                else:  
                    raise ValueError("Invalid prompt template setting. Choose from 'sentence_1', 'sentence_2', 'sentence_3', 'itemization', or 'total'.")
                    
                psudo_text_token = tokenizer(psudo_prompt)[0].to(image.device)       
                batch_psudo_prompts.append(psudo_text_token)
            stacked_psudo_prompts = torch.stack(batch_psudo_prompts)
            text_features, text_tokens = self.encode_text(stacked_psudo_prompts, normalize=True)
        else:
            text_features, text_tokens= self.encode_text(text, normalize=True) if text is not None else None

        if self.use_fusion and text_features is not None:
            text_features_l = text_tokens.clone() 
            text_features_g = text_tokens.clone().mean(axis=1)
            image_features_l = out_token.clone() 
            image_features_g = out_token.clone().mean(axis=1)             
            #text_features_l = text_features_l.permute(0,2,1)
            #image_features_l = image_features_l.permute(0,2,1)
            
            i2t_cls = self.fusion_model(torch.cat([image_features_g.unsqueeze(1), image_features_l], dim=1), text_features_g).squeeze(-1)
            t2i_cls = self.fusion_model(torch.cat([text_features_g.unsqueeze(1), text_features_l], dim=1), image_features_g).squeeze(-1)
        else:
            i2t_cls = None
            t2i_cls = None
            text_features_l = None
            text_features_g = None
            image_features_l = None
            image_features_g = None
        ...
        if self.output_dict:
            out_dict = {
                ...
                "tagging_words": tagging_words,
                "tag_logits": tag_logits,
                "i2t_cls": i2t_cls,
                "t2i_cls": t2i_cls,
                "text_features_l": text_features_l,
                "text_features_g": text_features_g,
                "image_features_l": image_features_l,
                "image_features_g": image_features_g,
            }
            if self.logit_bias is not None:
        ...
        return image_features, text_features, tag_logits, tagging_words, self.logit_scale.exp()
...
class CustomTextCLIP(nn.Module):
    ...
    def encode_image(self, image, normalize: bool = False):
        features, out_token = self.visual(image)
        return F.normalize(features, dim=-1) if normalize else features, out_token

...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.output_tokens:
            ...
            if self.proj is not None:
                B, N, C = tokens.shape
                tokens_flat = tokens.view(-1, C)  # [batch*seq, width]
                tokens_projected = tokens_flat @ self.proj  # [batch*seq, output_dim]
                tokens = tokens_projected.view(B, N, -1)  # [batch, seq, output_dim]
            
            # 프로젝션된 피처와 토큰 시퀀스 모두 반환
            return pooled, tokens  # ([batch, output_dim], [batch, seq, output_dim])
        
        # 기본 동작 (프로젝션된 피처만 반환)
        return pooled  # [batch, output_dim]

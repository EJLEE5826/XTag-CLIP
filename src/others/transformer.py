...
class VisionTransformer(nn.Module):
    ...
    def __init__(
            ...
            # add_learnable_tokens: bool = False,    # 추가: learnable token 적용 여부
            # n_learnable_tokens: int = 0,           # 추가: 추가할 learnable token 개수 (예: 1)
            # insert_position: int = 1               # 추가: 삽입 위치 (보통 class token 뒤)
    ):
        super().__init__()
        ...
        self.width = width
        # self.add_learnable_tokens = add_learnable_tokens
    ...
    def add_token(self):
        if self.add_learnable_tokens and self.n_learnable_tokens > 0:
            self.learnable_tokens = nn.Parameter(torch.randn(self.n_learnable_tokens, self.width))
            old_pe = self.positional_embedding
            old_seq_len = old_pe.shape[0]
            new_seq_len = old_seq_len + self.n_learnable_tokens
            new_pe = nn.Parameter(torch.zeros(new_seq_len, self.width))
            new_pe.data[:old_seq_len].copy_(old_pe.data)
            new_pe.data[old_seq_len:].copy_(torch.randn(self.n_learnable_tokens, self.width) * 0.02)
            self.positional_embedding = new_pe
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.add_learnable_tokens and self.n_learnable_tokens > 0:
            ltokens = self.learnable_tokens.unsqueeze(0).expand(x.shape[0], -1, -1).to(x.device)
            x = torch.cat([x[:, :self.insert_position, :], ltokens, x[:, self.insert_position:, :]], dim=1)
